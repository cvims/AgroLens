...
LUCAS_OUTPUT_FILE = '/home/ubuntu/Data/Datasets/Soil/processed/LUCAS/LUCAS_Soil_2018.csv'

# Nitrogen: NITKJD
# Phosphorus: PHETB1, PHETM3, PHETOL, PHPRTN, PHPTOT, PHPWSL
# pH: PHAQ, PHCA, PHKC, PHNF
# Required elements
elements = ['NITKJD','PHETB1','PHETM3','PHETOL','PHPTOT','PHPWSL','PHAQ','PHCA','PHKC','PHNF']
...
for element in elements:
    ...
    df1 = df.groupby(['longitude', 'latitude', 'date', 'continent', 'region', 'country_name'])['value_avg'].mean().reset_index().rename(columns={'longitude': 'TH_LONG', 'latitude': 'TH_LAT', 'date': 'SURVEY_DATE'})
...
df_combined_pH = df1.join(df2,how='outer',rsuffix='_PHCA').join(df3,how='outer',rsuffix='_PHKC').join(df4,how='outer',rsuffix='_PHNF')

# Calculate mean per row over PHCA, PHCA, PHNF and PHKC values
df_combined_pH['mean_pH'] = df_combined_pH[['value_avg', 'value_avg_PHCA', 'value_avg_PHKC', 'value_avg_PHNF']].mean(axis=1, skipna=True)
...
# Outer JOIN across PHAQ, PHCA, PHKC and PHNF 
df_combined_P = df1.join(df2,how='outer',rsuffix='_PHETM3').join(df3,how='outer',rsuffix='_PHETOL').join(df4,how='outer',rsuffix='_PHPTOT').join(df5,how='outer',rsuffix='_PHPWSL')

# Calculate mean per row over PHETB1, PHETM3, PHETOL, PHPTOT and PHPWSL values
df_combined_P['mean_P'] = df_combined_P[['value_avg', 'value_avg_PHETM3', 'value_avg_PHETOL', 'value_avg_PHPTOT', 'value_avg_PHPWSL']].mean(axis=1, skipna=True)
...
df_final = df_pH.merge(df_N, on=['TH_LONG', 'TH_LAT', 'SURVEY_DATE'], how='outer').merge(df_P, on=['TH_LONG', 'TH_LAT', 'SURVEY_DATE'], how='outer')
df_final.rename(columns={"mean_pH": "pH_H2O", "mean_P": "P", "continent_x": "continent", "region_x": "region", "country_name_x": "country_name"}, inplace=True)
...
df_combined_final[['TH_LONG', 'TH_LAT', 'SURVEY_DATE', 'continent', 'region', 'country_name', 'pH_CaCl2', 'pH_H2O', 'P', 'N', 'K']].to_csv(WOSIS_OUTPUT + 'WoSIS+LUCAS_Global.csv')
...
df_combined_final[['TH_LONG', 'TH_LAT', 'SURVEY_DATE', 'continent', 'region', 'country_name', 'pH_H2O', 'P', 'N']].to_csv(WOSIS_OUTPUT + 'WoSIS_Rest_of_World_without_Europe.csv')
...
df_WoSIS[df_WoSIS['continent']=='Northern America'].to_csv(WOSIS_OUTPUT  + 'WoSIS_Northern_America.csv')
df_WoSIS[df_WoSIS['continent']=='Oceania'].to_csv(WOSIS_OUTPUT + 'WoSIS_Oceania.csv')
df_WoSIS[df_WoSIS['continent']=='South America'].to_csv(WOSIS_OUTPUT + 'WoSIS_South America.csv')
