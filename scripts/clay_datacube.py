...
data_bands = pd.read_csv(filepath,  sep=",", usecols=range(idx_start, idx_end+1), header=0)
...
for i in range (0, data_bands_np.shape[0]):
    for j in range (0, 12):
        data_bands_clay[i,j,:,:] = data_bands_np[i,(0+m*j):(m+m*j)].reshape(9,9)
...
datacube = {
    ...
    "latlon": torch.tensor(
        latlong_clay, dtype=torch.float32, device=device
    ),
    "pixels": torch.tensor(
        data_bands_clay, dtype=torch.float32, device=device
    ),
